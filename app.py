...
def get_time_series_data(simulation_id: Optional[str] = None, streams: Optional[List] = None, 
                        start_time: Optional[float] = None, end_time: Optional[float] = None) -> Dict[str, Any]:
    """
    Get time series data from simulation results - dynamically discovers all available data
    """
    try:
        scenario_branch = sedaro.scenario(SCENARIO_BRANCH_VERSION_ID)
        sim = scenario_branch.simulation
        
        # Prepare results parameters
        results_params = {}
        if streams:
            results_params['streams'] = streams
        if start_time is not None:
            results_params['start'] = start_time
        if end_time is not None:
            results_params['stop'] = end_time
        
        if simulation_id:
            # Get specific simulation
            simulation_handle = sim.status(job_id=simulation_id)
            # Get results for the specific simulation
            results = simulation_handle.results(**results_params)
        else:
            # Get latest simulation
            simulation_handle = sim.status()
            # Get results for the latest simulation
            results = simulation_handle.results(**results_params)
        
        # Get summary
        summary = results.summarize()
        
        # Get time series data for each agent
        time_series_data = {}
        
        # Get agent names from summary
        agent_names = []
        if summary and isinstance(summary, dict):
            if 'Templated Agents' in summary:
                agent_names.extend(summary['Templated Agents'])
            if 'Peripheral Agents' in summary:
                agent_names.extend(summary['Peripheral Agents'])
        
        for agent_name in agent_names:
            agent = results.agent(agent_name)
            agent_data = {}
            
            # Dynamically discover all blocks and their time series data
            for block_name in agent.blocks:
                try:
                    block = agent.block(block_name)
                    block_data = {}
                    
                    # Get all attributes of the block that might contain time series data
                    block_attrs = dir(block)
                    
                    for attr_name in block_attrs:
                        # Skip internal attributes and methods
                        if attr_name.startswith('_') or callable(getattr(block, attr_name)):
                            continue
                            
                        try:
                            attr_value = getattr(block, attr_name)
                            
                            # Check if this attribute has a dataframe property (time series data)
                            if hasattr(attr_value, 'dataframe'):
                                try:
                                    df = attr_value.dataframe
                                    if df is not None and not df.empty:
                                        # Convert to records format
                                        records = df.to_dict('records')
                                        block_data[attr_name] = records
                                        print(f"Found time series data: {block_name}.{attr_name} ({len(records)} records)")
                                except Exception as e:
                                    print(f"Error converting {block_name}.{attr_name} to dataframe: {e}")
                            
                            # Check for nested attributes (like position.eci, position.lla)
                            elif hasattr(attr_value, '__dict__') or (hasattr(attr_value, '__getattr__') and not isinstance(attr_value, (str, int, float, bool))):
                                nested_data = {}
                                nested_attrs = dir(attr_value)
                                
                                for nested_attr in nested_attrs:
                                    if nested_attr.startswith('_') or callable(getattr(attr_value, nested_attr, None)):
                                        continue
                                        
                                    try:
                                        nested_value = getattr(attr_value, nested_attr)
                                        if hasattr(nested_value, 'dataframe'):
                                            try:
                                                df = nested_value.dataframe
                                                if df is not None and not df.empty:
                                                    nested_data[nested_attr] = df.to_dict('records')
                                                    print(f"Found nested time series data: {block_name}.{attr_name}.{nested_attr} ({len(df.to_dict('records'))} records)")
                                            except Exception as e:
                                                print(f"Error converting nested {block_name}.{attr_name}.{nested_attr} to dataframe: {e}")
                                    except Exception as e:
                                        continue
                                
                                if nested_data:
                                    block_data[attr_name] = nested_data
                                    
                        except Exception as e:
                            # Skip attributes that can't be accessed
                            continue
                    
                    # Only add block data if we found time series data
                    if block_data:
                        agent_data[block_name] = block_data
                        
                except Exception as e:
                    print(f"Error processing block {block_name} for {agent_name}: {e}")
                    continue
            
            time_series_data[agent_name] = agent_data
...
def get_available_time_series_streams(simulation_id: Optional[str] = None) -> Dict[str, Any]:
    """
    Get available time series streams from simulation results - dynamically discovers all available data
    """
    try:
        scenario_branch = sedaro.scenario(SCENARIO_BRANCH_VERSION_ID)
        sim = scenario_branch.simulation
        ...
        # Get summary to find available agents
        summary = results.summarize()
        
        # Get available agents and their blocks
        available_streams = {
            'agents': {},
            'categories': {
                'battery': [],
            ...
            for block_name in agent.blocks:
                try:
                    ...
                    block_data = {
                        ...
                        'type': 'block',
                        'category': 'other',
                        'description': f'Data from {block_name} block',
                        'components': []
                    }
                        ...
                        try:
                            ...
                            if hasattr(attr_value, 'dataframe'):
                                try:
                                    df = attr_value.dataframe
                                    if df is not None and not df.empty:
                                        block_data['components'].append(attr_name)
                                        # Auto-categorize based on attribute name
                                        if attr_name in ['soc', 'voltage', 'current']:
                                            block_data['category'] = 'battery'
                                        elif attr_name in ['power', 'powerConsumed']:
                                            block_data['category'] = 'power'
                                        elif attr_name in ['temperature']:
                                            block_data['category'] = 'thermal'
                                        elif attr_name in ['position']:
                                            block_data['category'] = 'position'
                                        elif attr_name in ['velocity']:
                                            block_data['category'] = 'velocity'
                                        elif attr_name in ['attitude']:
                                            block_data['category'] = 'attitude'
                                except Exception as e:
                                    print(f"Error checking {block_name}.{attr_name} for dataframe: {e}")
                                    ...
                                    try:
                                        ...
                                        if hasattr(nested_value, 'dataframe'):
                                            try:
                                                df = nested_value.dataframe
                                                if df is not None and not df.empty:
                                                    nested_components.append(f"{attr_name}.{nested_attr}")
                                            except Exception as e:
                                                print(f"Error checking nested {block_name}.{attr_name}.{nested_attr} for dataframe: {e}")
                                    except Exception as e:
                                        continue
                                
                                if nested_components:
                                    block_data['components'].extend(nested_components)
                                    # Auto-categorize based on attribute name
                                    if attr_name in ['position']:
                                        block_data['category'] = 'position'
                                    elif attr_name in ['velocity']:
                                        block_data['category'] = 'velocity'
                                    elif attr_name in ['attitude']:
                    ...
                    # Only add block data if we found time series data
                    if block_data['components']:
                        agent_data['available_data'].append(block_data)
                        
                except Exception as e:
                    print(f"Error processing block {block_name} for {agent_name}: {e}")
                    continue
            
            available_streams['agents'][agent_name] = agent_data
            
            # Categorize available data
            for data in agent_data['available_data']:
                available_streams['categories'][data['category']].append({
                    'agent': agent_name,
                    'name': data['name'],
                    'type': data['type'],
...
@app.route("/api/time_series_data", methods=['POST'])
def api_time_series_data():
    """API endpoint to get time series data"""
    try:
        data = request.get_json() or {}
        simulation_id = data.get('simulation_id')
        streams = data.get('streams')
        ...
        result = get_time_series_data(simulation_id, streams, start_time, end_time)
        return jsonify(result)
    except Exception as e:
        return jsonify({'error': str(e)}), 500

@app.route("/api/available_time_series_streams", methods=['GET'])
def api_available_time_series_streams():
    """API endpoint to get available time series streams"""
    try:
        simulation_id = request.args.get('simulation_id')
        result = get_available_time_series_streams(simulation_id)
        return jsonify(result)
    except Exception as e:
        return jsonify({'error': str(e)}), 500
